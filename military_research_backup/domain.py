...
def infer_action_type(description: str) -> str:
    ...
    if any(token in description for token in ["侦察", "侦搜", "侦收", "态势", "目标确认", "识别", "探测", "搜索", "跟踪", "光电", "频谱监测", "传感器"]) or "recon" in lower:
        return "recon"
    if any(token in description for token in ["压制", "打击", "火力", "摧毁", "点杀", "拦截", "击落", "硬杀伤", "防空导弹", "高射炮", "激光照射", "微波毁伤", "物理撞击"]) or "strike" in lower:
        return "strike"
    if any(token in description for token in ["机动", "突击", "突破", "前出", "登陆", "转移", "部署调整"]) or "mobility" in lower:
        return "mobility"
    if any(token in description for token in ["夺控", "稳控", "封控", "控制", "占领", "区域拒止"]) or "control" in lower:
        return "control"
    if any(token in description for token in ["补给", "恢复", "后送", "增援", "持续", "弹药补充", "战损修复"]) or "sustain" in lower:
        return "sustain"
    if any(token in description for token in ["指挥", "协同", "通信", "链路", "态势融合", "火力分配", "数据链"]) or lower.startswith("c2"):
        return "c2"
    if any(token in description for token in ["电子", "电磁", "干扰", "压制频谱", "软杀伤", "欺骗", "致盲", "数据链干扰", "GPS干扰"]) or lower == "ew":
        return "ew"
    return "other"
