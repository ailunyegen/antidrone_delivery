...
中的 System 1（参数化模板形变引擎）与 System 2（Delta 增量修补）：
...
4. 纯 Python 规则实现，零 LLM 调用，目标 < 1s 完成方案出库。

System 2（Delta Loop）：以应急预案为锚点，BottleneckDetector 定位最弱阶段，
LLM 仅对该阶段做局部补丁，门禁接受则更新、否则回滚，消除负向退化。

本模块是原型代码，不修改现有核心流水线（``engine.MilitaryResearchPipeline``）
的任何默认行为；所有新增功能需显式开关（fast_first / delta_refine 等）才生效。
"""
...
def _normalize_patch_action(action: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ``action_id`` / ``target`` / ``capability_weight`` 等语义字段若不并入
    description 会在下游解析时被静默丢弃，因此这里统一折叠进描述文本。
    """
...
class System1FastPlanner:
    ...
    def export(self, plan: CombatPlan, scenario: Scenario) -> Dict[str, Any]:
        """毫秒级出库：输出结构化方案摘要（不含 DoDAF/C2SIM 导出，依赖可选）。"""
        return {
            "title": plan.title,
            "phases": [
                {
    ...
    def warmup(self, scenario: Scenario | None = None) -> float:
        ...
        首次构建的 ~0.9s 主要来自 PyTorch context 初始化与 HOPE 权重装载。
        返回预热耗时（毫秒）。
        """
